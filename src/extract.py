...
def extract_text_and_regex(
    file_buffer, regex_pattern, save_markdown_to: str = "", save_pdf_to: str = ""
):
    ...
    logger.log(
        30, f"INFO: Extraction finished. Markdown length: {len(md_text)} characters."
    )

    if not md_text.strip():
        logger.log(
            40,
            "WARNING: Docling extraction returned empty text. Check the PDF content.",
        )
...
def parse_exam_schedule(text, semester_filter):
    ...
    day_pattern = re.compile(
        r"(Pazartesi|Salı|Çarşamba|Perşembe|Cuma|Cumartesi|Pazar)", re.IGNORECASE
    )
    ...
    for line in lines:
        ...
        if not cells or any(
            h in cells[0] for h in ["Tarih", "Ara Sınav", "Sınav Programı"]
        ):
            continue
        ...
        for i, cell in enumerate(cells):
            ...
            if time_match:
                if not data["time"]:
                    data["time"] = time_match.group().replace(" ", "")
                found = True
            ...
            if inst_match:
                if not data["instructor"]:
                    data["instructor"] = inst_match.group().strip()
                found = True
        ...
        unassigned = [
            i for i in range(len(cells)) if i not in assigned_cells and cells[i]
        ]
        ...
        for i in unassigned:
            ...
            if any(
                word in cell.upper()
                for word in ["DERSLİK", "AMFİ", "LAB", "BİNA", "MBG-", "SB-", "ODASI"]
            ):
                ...
                if any(word in cell for word in ["Prof", "Dr", "Gör", "Doç"]):
                    ...
                    data["location"] = (
                        (data["location"] + ", " + cell) if data["location"] else cell
                    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.warning(
            f"WARNING: Running through src/extract.py, outside of CLI. Selected {semester=}"
        )
        logger.info(
            "SAAT  TARİH(LER)  GÜN  | DERS ADI      | YERİ\n"
            "----  ----------- ---- | ------------- | --------------------"
        )
        for m in matches:
            logger.info(
                f"{m['time'][:5]} {m['date'][:5]:<11} {m['day'][:4]} | {m['course'][:13]:<13} | {m['location'][:35]}"
            )
